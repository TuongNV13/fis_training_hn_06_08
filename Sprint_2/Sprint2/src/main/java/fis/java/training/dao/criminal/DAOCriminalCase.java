package fis.java.training.dao.criminal;

import fis.java.training.dao.MemoryDataSource;
...
public class DAOCriminalCase implements IDAOCriminalCase {
    ...
    public void save(CriminalCase criminalCase) {
        if(
                !MemoryDataSource.CRIMINAL_CASES.stream().filter(item -> item.getId()==criminalCase.getId()).findFirst()
                        .isPresent()
        )
    ...
    public void update(CriminalCase criminalCase) {
        ...
        if (criminal.isPresent()){
            CriminalCase updateCriminalCase = criminal.get();
            updateCriminalCase.replaceWith(criminalCase);
        }
    }
